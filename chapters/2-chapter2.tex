...
The central research question of this thesis is an analysis of amulets recovered from burials across different social strata. To accurately differentiate these burials, this study employs a defined set of archaeological criteria. The differentiation proves particularly critical at sites where elite and non-elite burials were found together. Royal cemeteries offer the cleanest baseline for this distinction, as by being buried in a separate location, they were already considered exclusive. Therefore, it is highly unlikely that non-royal individuals would have been allowed to be buried within these explicitly royal cemeteries.


Previous Egyptological scholarship has established several frameworks for distinguishing social status based on burial context. Synthesizing these works, the key diagnostic criteria include: the scale of human and material investment in tomb construction' the quantity of grave goods; the presence of burial-specific objects (such as body containers, shabtis and canopics); the occurrence of inscriptions (both in the tomb and artifacts); and the use of prestigious materials \parencites{smithIntactTombsSeventeenth1992, richardsSocietyDeathAncient2005, wadaProvincialSocietyCemetery2007, podvinLéquipementTombesIntactes2017, lemosMaterialCultureSocial2017}.


Applying these criteria to the database---primarily to distinguish elite from non-elite burials within the same area---, we can better understand the social dynamics embedded burials practices. Table \ref{tab:social-group-division} correlates these variables across three dimensions: superstructure type, substructure type, and the median counts of total objects, burial-specific items, inscribed artifacts and those made of prestigious materials.\footnote{Prestigious materials are defined here as gold, silver, lapis lazuli, obsidian and ivory. While gold and ivory were obtainable in the Nubian territory, lapis lazuli and obsidian required long-distance importation from Afghanistan and Yemen, respectively. Silver, meanwhile, occurred in gold deposits or the Egyptian Eastern Desert. but could also be imported.} For this analysis, I employ the median rather than the average/mean. The average is misleading here because many tombs were completely empty, which pulls the data downward. The median, by contrast, identifies the middle value in the dataset and is therefore unaffected by the extremes, thereby providing a more accurate representation of how objects were distributed across contexts. 


The database records superstructure types---double pyramids, pyramids, tumuli, unknown (likely eroded pyramids), and no superstructure---, and three substructure types---chambers, cave tombs, and pits. A priori, one would expect that tombs requiring the greatest expenditure of labor and materials (both pyramid types and tombs with chambers) to yield the highest median counts across all object categories. Yet Table \ref{tab:social-group-division} reveals a more complex relationship. The effects of looting must be considered here, particularly since pyramids were the most visible and this the more frequently plundered structured.


Specifically, the combination of pyramids with chambers yielded the highest median for both total objects and inscribed items. Conversely, pyramids with pits produced very low medians across most categories, with the sole exception of total grave goods (7). Double pyramids, which appear exclusively with chambers, contained notably high medians for total objects (45) and inscribed objects (16), and scored highest among the remaining two categories (6 for burial-specific objects, and 8 for prestigious materials).


Tombs with unknown superstructure but chambers also showed high median total objects (27.5), though their other metrics remained low. Unknown superstructures with with cave tombs exhibited very low medians, likely due to the scarcity of this tomb type. Similarly, unknown superstructure with pits produced low medians across the board, except for the total object median of 5. The figures are close to that of pyramids with pits, which may suggest that some unknown superstructures were in fact pyramids. Tombs lacking a superstructure altogether, whether with chambers or pits, returned low medians--the highest value in this groups was total objects, with 3.5 for chambers and 7 for pits. Tumuli with pits also yielded low medians, though their total object median (4) exceeded that of no superstructures/chambers.


Taken together, these data suggest that a straightforward assignment of elite or non-elite status based on the correlation of structural and material criteria is not tenable. The problem of plunder also undermines any complete object-based analysis. Consequently, for the purposes of this thesis, I adopt a resource-expenditure approach. This means that tombs requiring the greatest investment of human and material resources are provisionally classified as elite, i.e., those that contained either a pyramid and/or chambers or cave tombs. At the same time, tombs with either no identifiable superstructure and tumuli paired with pits are classified as non-elite. This operational distinction is neither absolute nor self-evident, I will therefore revisit and refine it in Chapter 5, where the individual sites are subjected to a more contextual analysis.
...
{\small
	\begin{xltabular}{\textwidth}{X X X X X}
		...
		\toprule
		\textbf{Substructures} & \textbf{Median of total objects} & \textbf{Median of burial-specific objects} & \textbf{Median of inscribed objects} & \textbf{Median of prestigious material objects} \\
		\midrule
		\endhead
		\bottomrule
		\endlastfoot
